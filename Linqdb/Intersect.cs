...
namespace LinqDbInternal
{
    ...
    {
        public IDbQueryable<T> Intersect<T, TKey>(IDbQueryable<T> source, Expression<Func<T, TKey>> keySelector, HashSet<TKey> set)
        {
            if (set.Any(f => f == null))
            {
            ...
            if (keySelector != null)
            {
                ...
                name = SharedUtils.GetPropertyName(keySelector);
            }
            ...
            {
                new_set.Add(EncodeValue(table_info.Columns[name], s));
            }
        ...
        //    return Intersect(source, keySelector, new_set, source.LDBTree.QueryCache);
        //}
        //public IDbQueryable<T> Intersect<T, TKey>(IDbQueryable<T> source, Expression<Func<T, TKey>> keySelector, HashSet<decimal> set)
        //{
        //    CheckTableInfo<T>(source.Partition);
        //    var new_set = new List<EncodedValue>();
        //    foreach (var s in set)
        ...
        //    return Intersect(source, keySelector, new_set, source.LDBTree.QueryCache);
        //}
        //public IDbQueryable<T> Intersect<T, TKey>(IDbQueryable<T> source, Expression<Func<T, TKey>> keySelector, HashSet<long> set)
        //{
        //    CheckTableInfo<T>(source.Partition);
        //    var new_set = new List<EncodedValue>();
        //    foreach (var s in set)
        ...
        List<int> IntersectWithIndex(Oper odb, List<EncodedValue> val, ReadOptions ro, string snapshot_id, string id_snapshot_id)
        {
            ...
            switch (odb.ColumnType)
            {
                ...
                case LinqDbTypes.long_:
                    HashSet<long> lvals = new HashSet<long>(val.Where(f => !f.IsNull).Select(f => f.LongVal));
                    List<bool> bloom_long = new List<bool>(bloom_max);
                    for (int i = 0; i < bloom_max; i++)
                    {
                        bloom_long.Add(false);
                    }
                    foreach (var l_val in lvals)
                    {
                        bloom_long[Math.Abs(l_val.GetHashCode()) % bloom_max] = true;
                    }
                    ...
                    for (int i = 0; i < lcount; i++)
                    {
                        ...
                        {
                            int id = ids[j];
                            if (bloom_long[Math.Abs(lv[j].GetHashCode()) % bloom_max])
                            {
                                if (lvals.Contains(lv[j]))
                                {
                                    result.Add(id);
                                }
                            }
                ...
                case LinqDbTypes.decimal_:
                    HashSet<decimal> decvals = new HashSet<decimal>(val.Where(f => !f.IsNull).Select(f => f.DecimalVal));
                    List<bool> bloom_decimal = new List<bool>(bloom_max);
                    for (int i = 0; i < bloom_max; i++)
                    {
                        bloom_decimal.Add(false);
                    }
                    foreach (var d_val in decvals)
                    {
                        bloom_decimal[Math.Abs(d_val.GetHashCode()) % bloom_max] = true;
                    }
                    int deccount = index.Parts.Count();
                    for (int i = 0; i < deccount; i++)
                    {
                        ...
                        {
                            int id = ids[j];
                            if (bloom_decimal[Math.Abs(iv[j].GetHashCode()) % bloom_max])
                            {
                                if (decvals.Contains(iv[j]))
                                {
                                    result.Add(id);
                                }
                            }
