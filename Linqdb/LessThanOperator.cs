...
namespace LinqDbInternal
{
    public partial class Ldb
    {
        List<int> LessThanOperator(Oper odb, EncodedValue val, bool or_equal, Dictionary<long, byte[]> cache, ReadOptions ro, string snapshot_id, string id_snapshot_id)
        {
            ...
            if (val.Type == LinqDbTypes.double_ && val.DoubleVal < 0 || 
                val.Type == LinqDbTypes.decimal_ && val.DecimalVal < 0 || 
                val.Type == LinqDbTypes.DateTime_ && val.DoubleVal < 0 || 
                val.Type == LinqDbTypes.int_ && val.IntVal < 0 ||
                val.Type == LinqDbTypes.long_ && val.LongVal < 0)
            {
                return result_set;
            }
            else
