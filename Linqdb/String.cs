...
namespace LinqDbInternal
{
    public partial class Ldb
    {
        ...
        List<int> MakeSearch(string search_query, TableInfo table_info, string name, ReadOptions ro, bool partial, int? steps = null, int? start_step = null)
        {
            ...
            search_query = SharedUtils.NormaliseString(search_query);
            foreach (var s in search_query.Split().Where(f => !string.IsNullOrEmpty(f)))
            {
        ...
        double UpdateBitFunnelMemoryIndex(bool is_new, string value, int id, IndexNewData index_new, IndexChangedData index_changed, int bitfunnel_size)
        {
            var words = GetDistinctWords(value, false);

            var bits = new BitArray(bitfunnel_size);
            ...
            foreach (var w in words)
            {
                ...
                //var h4 = SharedUtils.HashBkdr(w, bitfunnel_size);
                //var h5 = SharedUtils.HashJenkins(w, bitfunnel_size);
            ...
            {
                index_new.Bits.Add(bits);
                index_new.Ids.Add(id);
            }
            if (!is_new && index_changed != null)
            {
                index_changed.BitValues[id] = bits;
            }

            return ratio;
        }
        int PhaseStep = 1000;
        void UpdateIndex(string old_val, string new_val, int id, WriteBatchWithConstraints batch, short ColumnNumber, int TableNumber, Dictionary<string, KeyValuePair<byte[], HashSet<int>>> cache, bool bySpacesOnly)
        {
            int phase = id / PhaseStep;
            if (old_val != null)
            {
                old_val = SharedUtils.NormaliseString(old_val);
            }
            if (new_val != null)
            {
                new_val = SharedUtils.NormaliseString(new_val);
            }
