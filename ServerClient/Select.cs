...
namespace LinqDbClientInternal
{
    public partial class Ldb
    {
        ...
        public ClientResult GetFromNamedQueue<T>()
        {
            var res = new ClientResult();
            res.Type = "getnamedqueue";
            return res;
        }
        ...
        public static List<R> CreateAnonymousType<R>(ServerResult server_res, Tuple<Dictionary<string, string>, Dictionary<string, short>> def, Ldb _db, List<string> props)
        {
            ...
            for (int i = 0; i < server_res.Count; i++)
            {
                ...
                foreach (var name in props)
                {
                    ...
                    if (type != LinqDbTypes.binary_ && type != LinqDbTypes.vector_ && type != LinqDbTypes.string_)
                    {
                        if (val == null)
                        {
                            switch (type)
                        ...
                        else
                        {
                            switch (type)
                            {
                                ...
                                case LinqDbTypes.int_:
                                    ...
                                    break;
                                case LinqDbTypes.long_:
                                    long long_v = BitConverter.ToInt64(MyReverseWithCopy(val), 0);
                                    cdata.Add(long_v);
                                    break;
                                default:
                                    throw new LinqDbException("Linqdb: Unsupported type");
            ...
            for (int i = 0; i < server_res.Count; i++)
            {
                ...
                if (server_res.IsOrdered)
                {
                    ...
                    foreach (var propInfo in def.Item2.OrderBy(f => f.Value).Select(f => f.Key))
                    {
                        ...
                        var value = cdata[j];
                        if (ps[propInfo].PropertyType == typeof(float[]) && value is byte[] bytes)
                        {
                            // Convert byte[] → float[]
                            var floats = FloatByteConverter.ByteArrayToFloatArray(bytes);
                            ps[propInfo].SetValue(cresult, floats);
                        }
                        else
                        {
                            // Set directly for all other types
                            ps[propInfo].SetValue(cresult, value);
                        }
                        j++;
                    }
