...
namespace LinqdbClient
{
    public class DistributedDb
    {
        ...
        /// <summary>
        ///  Indicates which distributed partitioned table operation is to be performed on.
        /// </summary>
        public ILinqDbDistributedPartitionedQueryable<T> DistributedPartitionedTable<T>(string partition) where T : new()
        {
            var res = new ILinqDbDistributedPartitionedQueryable<T>() { Partition = partition };
            res._dbs = _distributed_dbs;
            return res;
        }
