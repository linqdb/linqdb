...
namespace LinqDbInternal
{
    public partial class Ldb
    {
        public void CreateGroupByIndexString(string table_name, string group_name, string value_name)
        {
            ...
            lock (_write_lock)
            {
                if (bitfunnel_sizes.Any(f => f.Key.StartsWith(table_name + "|")))
                {
                ...
                using (var snapshot = leveld_db.CreateSnapshot())
                {
                    ...
                    if (table_info.Columns[value_name] == LinqDbTypes.binary_ || table_info.Columns[value_name] == LinqDbTypes.vector_ || table_info.Columns[value_name] == LinqDbTypes.string_)
                    {
                    ...
                    if (!existing_indexes.ContainsKey(table_name) || !existing_indexes[table_name].Contains("Id"))
                    {
                        ...
                        foreach (var id in ids)
                        {
                            if (counter % 1024 == 0)
                            {
                                cpart = new IndexPart() { IntValues = new List<int>(1024) };
                        ...
                        switch (table_info.Columns[value_name])
                        {
                            ...
                            case LinqDbTypes.long_:
                                ...
                                if ((ids.Any() ? ids.Max() : 0) < 250000000)
                                {
                                    ...
                                    {
                                        throw new LinqDbException("Linqdb: column " + index.ColumnName + " has gaps in data. Prior to building an index gaps must be updated with values. (" + totalexlong + " != " + ids.Count() + ")");
                                    }
                            ...
                            case LinqDbTypes.decimal_:
                                ...
                                if ((ids.Any() ? ids.Max() : 0) < 250000000)
                                {
                                    ...
                                    {
                                        throw new LinqDbException("Linqdb: column " + index.ColumnName + " has gaps in data. Prior to building an index gaps must be updated with values. (" + totalexdec + " != " + ids.Count() + ")");
                                    }
        ...
        public void CreateGroupByIndex<T, TKey1, TKey2>(Expression<Func<T, TKey1>> groupPropertySelector, Expression<Func<T, TKey2>> valuePropertySelector)
        {
            ...
            CreateGroupByIndexString(type_name, group_name, value_name);
            AddIndexToFile(type_name, group_name, value_name, false, 0, 0);
        }
