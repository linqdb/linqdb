...
namespace ServerSharedData
{
        ...
        public static byte[] GetBytes(Command comm, bool no_prefix = false)
        {
            ...
            if (comm.Commands != null && comm.Commands.Any())
            {
                ...
                foreach (var cr in comm.Commands)
                {
                    ...
                    if (cr.FromDecimal == 0 && cr.ToDecimal == 0)
                    {
                        result.Add(0);
                    }
                    else
                    {
                        result.Add(1);
                        result.AddRange(BitConverter.GetBytes(cr.Boundaries));
                        result.AddRange(SharedUtils.DecimalConversion.ToByteArray(cr.FromDecimal));
                        result.AddRange(SharedUtils.DecimalConversion.ToByteArray(cr.ToDecimal));
                    }
                    if ((cr.Int_set == null || !cr.Int_set.Any()) && !cr.Int_null &&
                       (cr.Double_set == null || !cr.Double_set.Any()) && !cr.Double_null &&
                       (cr.Decimal_set == null || !cr.Decimal_set.Any()) && !cr.Decimal_null &&
                       (cr.Long_set == null || !cr.Long_set.Any()) && !cr.Long_null &&
                    ...
                    else
                    {
                        ...
                        if (cr.Decimal_set != null && cr.Decimal_set.Any())
                        {
                            result.AddRange(BitConverter.GetBytes(cr.Decimal_set.Count()));
                            foreach (var item in cr.Decimal_set)
                            {
                                result.AddRange(SharedUtils.DecimalConversion.ToByteArray(item));
                            }
                        }
                        else
                        {
                        ...
                        result.Add(cr.Decimal_null ? (byte)1 : (byte)0);


                        if (cr.Long_set != null && cr.Long_set.Any())
                        {
                            result.AddRange(BitConverter.GetBytes(cr.Long_set.Count()));
                            foreach (var item in cr.Long_set)
                            {
                                result.AddRange(BitConverter.GetBytes(item));
                            }
                        }
                        ...
                        result.Add(cr.Long_null ? (byte)1 : (byte)0);
        ...
        public static Command GetCommands(byte[] comms)
        {
            ...
            for (int z = 0; z < rcount; z++)
            {
                ...
                if (len != 0)
                {
                    cr.Boundaries = BitConverter.ToInt16(new byte[2] { comms[current], comms[current + 1] }, 0);
                    current += 2;
                    cr.FromDecimal = SharedUtils.DecimalConversion.FromByteArray(new byte[32] { comms[current], comms[current + 1], comms[current + 2], comms[current + 3], comms[current + 4], comms[current + 5], comms[current + 6], comms[current + 7], comms[current + 8], comms[current + 9], comms[current + 10], comms[current + 11], comms[current + 12], comms[current + 13], comms[current + 14], comms[current + 15], comms[current + 16], comms[current + 17], comms[current + 18], comms[current + 19], comms[current + 20], comms[current + 21], comms[current + 22], comms[current + 23], comms[current + 24], comms[current + 25], comms[current + 26], comms[current + 27], comms[current + 28], comms[current + 29], comms[current + 30], comms[current + 31]});
                    current += 32;
                    cr.ToDecimal = SharedUtils.DecimalConversion.FromByteArray(new byte[32] { comms[current], comms[current + 1], comms[current + 2], comms[current + 3], comms[current + 4], comms[current + 5], comms[current + 6], comms[current + 7], comms[current + 8], comms[current + 9], comms[current + 10], comms[current + 11], comms[current + 12], comms[current + 13], comms[current + 14], comms[current + 15], comms[current + 16], comms[current + 17], comms[current + 18], comms[current + 19], comms[current + 20], comms[current + 21], comms[current + 22], comms[current + 23], comms[current + 24], comms[current + 25], comms[current + 26], comms[current + 27], comms[current + 28], comms[current + 29], comms[current + 30], comms[current + 31] });
                    current += 32;
                }
                len = BitConverter.ToInt16(new byte[2] { comms[current], 0 }, 0);
                current++;
                if (len != 0)
                {
                    ...
                    if (ilen != 0)
                    {
                        cr.Decimal_set = new HashSet<decimal>();
                        for (int h = 0; h < ilen; h++, current += 32)
                        {
                            cr.Decimal_set.Add(SharedUtils.DecimalConversion.FromByteArray(new byte[32] { comms[current], comms[current + 1], comms[current + 2], comms[current + 3], comms[current + 4], comms[current + 5], comms[current + 6], comms[current + 7], comms[current + 8], comms[current + 9], comms[current + 10], comms[current + 11], comms[current + 12], comms[current + 13], comms[current + 14], comms[current + 15], comms[current + 16], comms[current + 17], comms[current + 18], comms[current + 19], comms[current + 20], comms[current + 21], comms[current + 22], comms[current + 23], comms[current + 24], comms[current + 25], comms[current + 26], comms[current + 27], comms[current + 28], comms[current + 29], comms[current + 30], comms[current + 31] }));
                        }
                    }

                    cr.Decimal_null = comms[current] == 1;
                    current++;
                    ...
                    if (ilen != 0)
                    {
                        cr.Long_set = new HashSet<long>();
                        for (int h = 0; h < ilen; h++, current += 8)
                        {
                            cr.Long_set.Add(BitConverter.ToInt64(new byte[8] { comms[current], comms[current + 1], comms[current + 2], comms[current + 3], comms[current + 4], comms[current + 5], comms[current + 6], comms[current + 7] }, 0));
                        }
                    }

                    cr.Long_null = comms[current] == 1;
                    current++;
    ...
    public class ClientResult
    {
        ...
        public HashSet<decimal> Decimal_set { get; set; }
        public bool Decimal_null { get; set; }
        public HashSet<long> Long_set { get; set; }
        public bool Long_null { get; set; }
