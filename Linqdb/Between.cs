...
namespace LinqDbInternal
{
    ...
    {
        public IDbQueryable<T> Between<T, TKey>(IDbQueryable<T> source, Expression<Func<T, TKey>> keySelector, object from, object to, BetweenBoundariesInternal boundaries = BetweenBoundariesInternal.BothInclusive)
        {
        ...
        public List<int> FindOneBetween(BetweenInfo bet, Dictionary<long, byte[]> cache, ReadOptions ro)
        {
            ...
            if (bet.ColumnType != LinqDbTypes.decimal_ && Convert.ToDouble(bet.From) >= 0 && Convert.ToDouble(bet.To) >= 0 ||
                bet.ColumnType == LinqDbTypes.decimal_ && Convert.ToDecimal(bet.From) >= 0 && Convert.ToDecimal(bet.To) >= 0)
            { 
                var op  = new Oper()
                {
                    ColumnNumber = bet.ColumnNumber,
            ...
            else if (bet.ColumnType != LinqDbTypes.decimal_ && Convert.ToDouble(bet.From) < 0 && Convert.ToDouble(bet.To) < 0 ||
                     bet.ColumnType == LinqDbTypes.decimal_ && Convert.ToDecimal(bet.From) < 0 && Convert.ToDecimal(bet.To) < 0)
            {
                var op  = new Oper()
                {
                ...
                else if (val.Type == LinqDbTypes.decimal_)
                {
                    val.DecimalVal *= -1;
                }
                ...
                else if (val.Type == LinqDbTypes.long_)
                {
                    val.LongVal *= -1;
                }
                var result_tmp = GreaterThanOperator(op, val, bet.BetweenBoundaries == BetweenBoundariesInternal.BothInclusive || bet.BetweenBoundaries == BetweenBoundariesInternal.FromExclusiveToInclusive, cache, ro, snapshot_id, id_snapshot_id,
                                                     bet.ColumnType != LinqDbTypes.decimal_ ? (object)(-1 * Convert.ToDouble(bet.From)) : (object)(-1 * Convert.ToDecimal(bet.From)), bet.BetweenBoundaries == BetweenBoundariesInternal.BothInclusive || bet.BetweenBoundaries == BetweenBoundariesInternal.FromInclusiveToExclusive);
                result = result_tmp;
            }
            else
            {
