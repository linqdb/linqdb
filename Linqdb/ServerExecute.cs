    ...
    public partial class Ldb
    {
        ...
        public ServerResult Execute(Command comm, WriteBatchWithConstraints trans_batch, Dictionary<string, int> trans_count_cache, Dictionary<string, KeyValuePair<TableInfo, Dictionary<string, KeyValuePair<byte[], HashSet<int>>>>> scache)
        {
            ...
            if (comm.Type == (int)CommandType.GetAllTables)
            {
                ...
                server_result.TableInfo = sb.ToString().Trim("|".ToCharArray());
                return server_result;
            }
            ...
            foreach (var inst in comm.Commands)
            {
                if (inst.Type == "where")
                {
                    ...
                    foreach (var op in inst.Opers.OrderBy(f => f.Id))
                    {
                        ...
                        if (op.NonDbValue != null)
                        {
                            ...
                            else if (table_info.Columns[op.ColumnName] == LinqDbTypes.long_)
                            {
                                ndb = BitConverter.ToInt64(op.NonDbValue, 0);
                            }
                            else if (table_info.Columns[op.ColumnName] == LinqDbTypes.double_)
                            {
                                ndb = BitConverter.ToDouble(op.NonDbValue, 0);
                            }
                            else if (table_info.Columns[op.ColumnName] == LinqDbTypes.decimal_)
                            {
                                ndb = DecimalConversion.FromByteArray(op.NonDbValue);
                            }
                            ...
                            else if (table_info.Columns[op.ColumnName] == LinqDbTypes.binary_ || table_info.Columns[op.ColumnName] == LinqDbTypes.vector_)
                            {
                                ndb = op.NonDbValue;
                            }
                        }
                        if (op.ColumnName != null)
                        {
                            ...
                            {
                                nop.ColumnType = LinqDbTypes.long_;
                            }
                            else if (table_info.Columns[op.ColumnName] == LinqDbTypes.double_)
                            {
                            ...
                            {
                                nop.ColumnType = LinqDbTypes.decimal_;
                            }
                            else if (table_info.Columns[op.ColumnName] == LinqDbTypes.DateTime_)
                            {
                            ...
                            else if (table_info.Columns[op.ColumnName] == LinqDbTypes.vector_)
                            {
                                nop.ColumnType = LinqDbTypes.vector_;
                            }
                        }
                ...
                else if (inst.Type == "betweend")
                {
                    ...
                    tree.BetweenInfo.Add(info);
                    info.BetweenBoundaries = (BetweenBoundariesInternal)inst.Boundaries;
                    info.From = inst.FromDecimal;
                    info.To = inst.ToDecimal;
                ...
                else if (inst.Type == "intersect")
                {
                    ...
                    else if (inst.Decimal_set != null || inst.Decimal_null)
                    {
                        if (inst.Decimal_set != null)
                        {
                            foreach (var s in inst.Decimal_set)
                            {
                                new_set.Add(EncodeValue(LinqDbTypes.decimal_, s));
                            }
                        }
                        if (inst.Decimal_null)
                        {
                            new_set.Add(EncodeValue(LinqDbTypes.decimal_, null));
                        }
                    }
                    else if (inst.Long_set != null || inst.Long_null)
                    {
                        if (inst.Long_set != null)
                        {
                            foreach (var s in inst.Long_set)
                            {
                                new_set.Add(EncodeValue(LinqDbTypes.long_, s));
                            }
                        }
                        if (inst.Long_null)
                        {
                            new_set.Add(EncodeValue(LinqDbTypes.long_, null));
                        }
                    }
                ...
                else if (inst.Type == "searchneighbours")
                {
                    CheckTableInfo(comm.TableDef, comm.TableName, CommandHelper.CanWrite(comm.User, comm.Pass));
                    ...
                    var info = new SearchInfo();
                    info.Vector = SharedUtils.FloatByteConverter.ByteArrayToFloatArray(inst.QueueData);
                    tree.SearchInfo.Add(info);
                    var name = inst.Selector;
                    var table_info = GetTableInfo(comm.TableName);
                    info.TableInfo = table_info;
                    info.Name = name;
                    info.Steps = inst.Take;
                    info.IsVector = true;
                ...
                else if (inst.Type.StartsWith("getnamedqueue"))
                {
                    if (!CommandHelper.CanRead(comm.User, comm.Pass))
                    {
                        throw new LinqDbException("Linqdb: insufficient permissions for the user.");
                    }
                    server_result.QueueData = GetNamedQueue(comm, inst.Selector, inst.Int_null);
                }
                ...
                else if (inst.Type == "namedqueue")
                {
                    if (!CommandHelper.CanWrite(comm.User, comm.Pass))
                    {
                        throw new LinqDbException("Linqdb: insufficient permissions for the user.");
                    }
                    SaveNamedQueue(inst.QueueData, comm, inst.Selector);
                }
                ...
                else if (inst.Type.StartsWith("propertyindex"))
                {
                    if (!CommandHelper.CanRead(comm.User, comm.Pass))
                    {
                        throw new LinqDbException("Linqdb: insufficient permissions for the user.");
                    ...
                    bool is_bitfunnel = false;
                    int bitfunnel_size = 0;
                    int batch_size = 0;
                    if (inst.String_null)
                    {
                        is_bitfunnel = true;
                        bitfunnel_size = (int)inst.Take;
                        batch_size = (int)inst.Skip;
                    }

                    CheckTableInfo(comm.TableDef, comm.TableName, CommandHelper.CanWrite(comm.User, comm.Pass));
                    var ratio = CreatePropIndex(comm.TableName, inst.Selector, is_bitfunnel, bitfunnel_size, batch_size);
                    server_result.Count = (int)(Math.Round(ratio, 2) * 100);
                    AddIndexToFile(comm.TableName, null, inst.Selector, is_bitfunnel, bitfunnel_size, batch_size);
                }
                else if (inst.Type.StartsWith("removepropertyindex"))
                {
                    if (!CommandHelper.CanRead(comm.User, comm.Pass))
                    {
                        throw new LinqDbException("Linqdb: insufficient permissions for the user.");
                    ...
                    bool is_bitfunnel = false;
                    if (inst.Type.Contains("-"))
                    {
                        is_bitfunnel = true;
                    }

                    CheckTableInfo(comm.TableDef, comm.TableName, CommandHelper.CanWrite(comm.User, comm.Pass));
                    RemoveIndexFromFile(comm.TableName, null, inst.Selector, is_bitfunnel);
                    RemoveIndex(comm.TableName, null, inst.Selector, is_bitfunnel);
                }
                else if (inst.Type == "groupindex")
                {
                    ...
                    AddIndexToFile(comm.TableName, inst.Selector, inst.Query, false, 0, 0);
                }
        ...
        List<int> SaveItems(List<BinData> items, Dictionary<string, short> TableDefOrder, WriteBatchWithConstraints batch, TableInfo table_info, string table_name, Dictionary<string, int> trans_count_cache, Dictionary<string, KeyValuePair<byte[], HashSet<int>>> string_cache, Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>> memory_index_meta, bool is_trans, List<int> new_ids = null)
        {
            ...
            foreach (var item in items)
            {
                ...
                foreach (var p in table_info.Columns)
                {
                    ...
                    else if (p.Value == LinqDbTypes.long_)
                    {
                        if (item.Bytes[TableDefOrder[p.Key]].Any())
                        {
                            value = BitConverter.ToInt64(item.Bytes[TableDefOrder[p.Key]], 0);
                        }
                    }
                    ...
                    else if (p.Value == LinqDbTypes.decimal_)
                    {
                        if (item.Bytes[TableDefOrder[p.Key]].Any())
                        {
                            value = DecimalConversion.FromByteArray(item.Bytes[TableDefOrder[p.Key]]);
                        }
                    }
        ...
        void UpdateItems(UpdateInfo info, Dictionary<int, byte[]> values, TableInfo table_info, WriteBatchWithConstraints batch, Dictionary<string, KeyValuePair<byte[], HashSet<int>>> string_cache, Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>> memory_index_meta, bool is_trans)
        {
            var vector_cache = new Dictionary<string, VamanaIndex>();
            foreach (var item in values)
            {
                ...
                else if (info.ColumnType == LinqDbTypes.long_)
                {
                    if (item.Value != null)
                    {
                        value = BitConverter.ToInt64(item.Value, 0);
                    }
                }
                ...
                else if (info.ColumnType == LinqDbTypes.decimal_)
                {
                    if (item.Value != null)
                    {
                        value = DecimalConversion.FromByteArray(item.Value);
                    }
                }
                ...
                if (info.ColumnType == LinqDbTypes.string_)
                {
                    ...
                    SaveStringData(batch, (string)value, info.ColumnName, info.TableInfo, item.Key, string_cache, false, index_new, index_changed, bitfunnel_size);
                }
                else if (info.ColumnType == LinqDbTypes.binary_ || info.ColumnType == LinqDbTypes.vector_)
                {
        ...
        void SaveNamedQueue(byte[] item, Command comm, string name)
        {
            Queue.PutToQueueAndReplace(item, comm.TableName+"-"+name);
        }
        ...
        byte[] GetNamedQueue(Command comm, string name, bool delete)
        {
            return Queue.GetAllFromQueueAndLeave(comm.TableName+"-"+name, delete);
        }
        ...
        void UpdateServer(Dictionary<int, byte[]> UpdateData, Command comm, string selector, WriteBatchWithConstraints trans_batch, Dictionary<string, KeyValuePair<TableInfo, Dictionary<string, KeyValuePair<byte[], HashSet<int>>>>> scache)
        {
            if (UpdateData != null && UpdateData.Any())
            {
                    ...
                    try
                    {
                        ...
                        if (_update_data.valuesServer.Any())
                        {
                            using (WriteBatchWithConstraints batch = new WriteBatchWithConstraints())
                            {
                                ...
                                UpdateItems(info, _update_data.valuesServer, table_info, batch, string_cache, meta_index, false);
                                WriteStringCacheToBatch(batch, string_cache, table_info, null);
                                var snapshots_dic = InsertIndexChanges(table_info, meta_index);
                                foreach (var snap in snapshots_dic)
                                {
                ...
                else
                {
                    ...
                    Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>> meta_index = BuildMetaOnIndex(table_info);
                    UpdateItems(info, UpdateData, table_info, trans_batch, scache[table_info.Name].Value, meta_index, true);
                    var snapshots_dic = InsertIndexChanges(table_info, meta_index);
                    foreach (var snap in snapshots_dic)
                    {
                        var skey = MakeSnapshotKey(table_info.TableNumber, table_info.ColumnNumbers[snap.Key]);
        ...
        void DeleteServer(HashSet<int> DeleteIds, Command comm, WriteBatchWithConstraints trans_batch, Dictionary<string, int> trans_count_cache, Dictionary<string, KeyValuePair<TableInfo, Dictionary<string, KeyValuePair<byte[], HashSet<int>>>>> scache)
        {
            if (DeleteIds != null && DeleteIds.Any())
            {
                    ...
                    try
                    {
                        ...
                        if (_delete_data.ids.Any())
                        {
                            using (WriteBatchWithConstraints batch = new WriteBatchWithConstraints())
                            {
                                var string_cache = new Dictionary<string, KeyValuePair<byte[], HashSet<int>>>();
                                Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>> meta_index = BuildMetaOnIndex(table_info);
                                Deleteitems(_delete_data.ids, batch, table_info, trans_count_cache, string_cache, meta_index, false);
                                WriteStringCacheToBatch(batch, string_cache, table_info, null);
                                var snapshots_dic = InsertIndexChanges(table_info, meta_index);
                                foreach (var snap in snapshots_dic)
                                {
                ...
                else
                {
                    ...
                    Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>> meta_index = BuildMetaOnIndex(table_info);
                    Deleteitems(DeleteIds, trans_batch, table_info, trans_count_cache, scache[table_info.Name].Value, meta_index, true);
                    var snapshots_dic = InsertIndexChanges(table_info, meta_index);
                    foreach (var snap in snapshots_dic)
                    {
                        var skey = MakeSnapshotKey(table_info.TableNumber, table_info.ColumnNumbers[snap.Key]);
        ...
        void SelectServer(Command comm, IDbQueryable<object> iq, ServerResult server_result, string type, List<string> anonSelect)
        {
            ...
            using (var snapshot = leveld_db.CreateSnapshot())
            {
                ...
                if (distances != null)
                {
                    ...
                    {
                        dist[distances.ResIds[i]] = distances.ResDistances[i];
                    }
                    server_result.QueueData = SimpleIntDoubleCodec.ToBytes(dist);
                }
        ...
        void Deleteitems(HashSet<int> ids, WriteBatchWithConstraints batch, TableInfo table_info, Dictionary<string, int> trans_count_cache, Dictionary<string, KeyValuePair<byte[], HashSet<int>>> string_cache, Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>> memory_index_meta, bool is_trans)
        {
            ...
            foreach (var id in ids)
            {
                ...
                {
                    if (column.Item2 != LinqDbTypes.binary_ && column.Item2 != LinqDbTypes.string_ && column.Item2 != LinqDbTypes.vector_)
                    {
