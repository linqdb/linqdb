...
namespace LinqDbInternal
{
    ...
    {
        List<int> GreaterThanOperator(Oper odb, EncodedValue val, bool is_equal, Dictionary<long, byte[]> cache, ReadOptions ro, string snapshot_id, string id_snapshot_id, object stop_val = null, bool stop_equal = false)
        {
            ...
            if (val.Type == LinqDbTypes.double_ && val.DoubleVal < 0 || val.Type == LinqDbTypes.decimal_ && val.DecimalVal < 0 || val.Type == LinqDbTypes.DateTime_ && val.DoubleVal < 0 || val.Type == LinqDbTypes.int_ && val.IntVal < 0 || val.Type == LinqDbTypes.long_ && val.LongVal < 0)
            {
            ...
            else
            {
                ...
                using (var it = leveld_db.NewIterator(null, ro))
                {
                    ...
                    if (stop_val != null)
                    {
                        ...
                        {
                            cv = BitConverter.ToInt32(kinfo.Val.MyReverseWithCopy(LinqDbTypes.int_), 0);
                            if (cv == Convert.ToDouble(stop_val) && !stop_equal || cv > Convert.ToDouble(stop_val))
                            {
                                return result_set;
                            }
                        }
                        if (val.Type == LinqDbTypes.long_)
                        {
                            cv = BitConverter.ToInt64(kinfo.Val.MyReverseWithCopy(LinqDbTypes.long_), 0);
                            if (cv == Convert.ToDouble(stop_val) && !stop_equal || cv > Convert.ToDouble(stop_val))
                            {
                                return result_set;
                            }
                        ...
                        {
                            cv = BitConverter.ToDouble(kinfo.Val.MyReverseWithCopy(LinqDbTypes.double_), 0);
                            if (cv == Convert.ToDouble(stop_val) && !stop_equal || cv > Convert.ToDouble(stop_val))
                            {
                                return result_set;
                            }
                        }
                        if (val.Type == LinqDbTypes.decimal_)
                        {
                            cvdec = DecimalConversion.FromByteArray(kinfo.Val);
                            if (cvdec == Convert.ToDecimal(stop_val) && !stop_equal || cvdec > Convert.ToDecimal(stop_val))
                            {
                                return result_set;
                            }
                        }
        ...
        List<int> GreaterThanNegativeOperator(Oper odb, EncodedValue val, bool is_equal, Dictionary<long, byte[]> cache, ReadOptions ro, string snapshot_id, string id_snapshot_id)
        {
            ...
            var result_set = new List<int>();
            if (val.Type == LinqDbTypes.double_ && val.DoubleVal >= 0 || val.Type == LinqDbTypes.DateTime_ && val.DoubleVal >= 0 || val.Type == LinqDbTypes.int_ && val.IntVal >= 0 || val.Type == LinqDbTypes.decimal_ && val.DecimalVal >= 0 || val.Type == LinqDbTypes.long_ && val.LongVal >= 0)
            {
                return result_set;
            }
            else
        ...
        List<int> GreaterThanOperatorWithIndex(Oper odb, EncodedValue val, bool is_equal, ReadOptions ro, string snapshot_id, string id_snapshot_id, object stop_val = null, bool stop_equal = false)
        {
            ...
            switch (odb.ColumnType)
            {
                case LinqDbTypes.int_:
                    ...
                    else
                    {
                        ...
                        {
                            int stop_val_int = Convert.ToInt32(stop_val);
                            if (!stop_equal)
                            {
                                int icount = index.Parts.Count();
                                for (int i = 0; i < icount; i++)
                ...
                case LinqDbTypes.long_:
                    ...
                    else
                    {
                        ...
                        {
                            long stop_val_long = Convert.ToInt64(stop_val);
                            if (!stop_equal)
                            {
                                int icount = index.Parts.Count();
                                for (int i = 0; i < icount; i++)
                                {
                                    ...
                                    for (int j = 0; j < jcount; j++)
                                    {
                                        if (lval < lv[j] && lv[j] < stop_val_long)
                                        {
                                            result.Add((int)ids[j]);
                                        }
                                    }
                ...
                case LinqDbTypes.DateTime_:
                    ...
                    else
                    {
                        ...
                        {
                            double stop_val_double = Convert.ToDouble(stop_val);
                            if (!stop_equal)
                            {
                                int icount = index.Parts.Count();
                                for (int i = 0; i < icount; i++)
                ...
                case LinqDbTypes.decimal_:
                    ...
                    else
                    {
                        ...
                        {
                            decimal stop_val_double = Convert.ToDecimal(stop_val);
                            if (!stop_equal)
                            {
                                int icount = index.Parts.Count();
                                for (int i = 0; i < icount; i++)
    ...
    {
        public static byte[] MyReverseNoCopy(this byte[] val, LinqDbTypes type)
        {
            ...
            if ((type == LinqDbTypes.double_ || type == LinqDbTypes.DateTime_) && val[7] == (byte)128)
            {
            ...
            if ((type == LinqDbTypes.double_ || type == LinqDbTypes.DateTime_) && val[7] == (byte)255 && val[6] == (byte)248)
            {
            ...
            //negative infinity
            if ((type == LinqDbTypes.double_ || type == LinqDbTypes.DateTime_) && val[7] == (byte)255 && val[6] == (byte)240)
            {
                if (val[0] == (byte)0 && val[1] == (byte)0 && val[2] == (byte)0 && val[3] == (byte)0 && val[4] == (byte)0 && val[5] == (byte)0)
                {
            ...
            //positive infinity
            if ((type == LinqDbTypes.double_ || type == LinqDbTypes.DateTime_) && val[7] == (byte)127 && val[6] == (byte)240)
            {
                if (val[0] == (byte)0 && val[1] == (byte)0 && val[2] == (byte)0 && val[3] == (byte)0 && val[4] == (byte)0 && val[5] == (byte)0)
                {
        ...
        public static byte[] MyReverseWithCopy(this byte[] val, LinqDbTypes type)
        {
            if (val == null || val.Length == 0)
            {
                return val;
