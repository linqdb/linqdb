            ...
            if (!no)
            {
                ...
                try
                {
                    ...
                    using (WriteBatchWithConstraints batch = new WriteBatchWithConstraints())
                    {
                        ...
                        var index_data = new List<Tuple<TableInfo, Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>>>>();
                        foreach (var save_list in _trans_data.data_to_save)
                        {
                            ...
                            Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>> meta_index = ldb.BuildMetaOnIndex(save_list.Value.Key);
                            ...
                            index_data.Add(new Tuple<TableInfo, Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>>>(save_list.Value.Key, meta_index));
                        }
                        foreach (var update_list in _trans_data.data_to_update)
                        {
                            foreach (var update_field in update_list.Value)
                            {
                                ...
                                Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>> meta_index = ldb.BuildMetaOnIndex(update_field.Key.TableInfo);
                                ldb.UpdateBatch(update_field.Key, update_field.Value, update_field.Key.TableInfo, batch, sc[update_field.Key.TableInfo.Name].Value, meta_index, true);
                                var snapshots_dic = ldb.InsertIndexChanges(update_field.Key.TableInfo, meta_index);
                                index_data.Add(new Tuple<TableInfo, Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>>>(update_field.Key.TableInfo, meta_index));
                            }
                        ...
                        foreach (var delete_list in _trans_data.data_to_delete)
                        {
                            ...
                            Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>> meta_index = ldb.BuildMetaOnIndex(delete_list.Value.Key);
                            ldb.DeleteBatch(delete_list.Value.Value, delete_list.Value.Key, batch, trans_count_cache, sc[delete_list.Value.Key.Name].Value, meta_index, true);
                            index_data.Add(new Tuple<TableInfo, Dictionary<string, Tuple<IndexNewData, IndexDeletedData, IndexChangedData, int, int>>>(delete_list.Value.Key, meta_index));
                        }
