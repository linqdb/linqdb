...
{
    public class DistributedPartitionedNoId : ITestDistributed
    {
        public void Do(DistributedDb db)
        {
            ...
            try
            {
                db.DistributedPartitionedTable<NoIdType>(partition).Save(d);
            }
            catch (Exception ex)
            {
                ex_msg = ex.Message;
            ...
            try
            {
                db.DistributedPartitionedTable<NoSidType>(partition).Save(d2);
            }
            catch (Exception ex)
            {
                ex_msg = ex.Message;
