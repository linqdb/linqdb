...
namespace LinqdbClient
{
    ...
    public class Db
    {
        ...
        /// <summary>
        ///  Indicates which named queue operation is to be performed on.
        /// </summary>
        public ILinqDbNamedQueueQueryable<T> NamedQueue<T>(string name) where T : new()
        {
            var _inter = new IDbNamedQueueQueryable<T>() { Result = new List<ClientResult>(), _db = this._db };
            return new ILinqDbNamedQueueQueryable<T>() { _internal = _inter, Name = name };
        }
        /// <summary>
        ...
        /// <summary>
        ///  Indicates which partitioned table operation is to be performed on.
        /// </summary>
        public ILinqDbPartitionedQueryable<T> PartitionedTable<T>(string partition) where T : new()
        {
            var _inter = new IDbQueryable<T>() { Result = new List<ClientResult>(), _db = this._db, Partition = partition };
            return new ILinqDbPartitionedQueryable<T>() { _queryable = new ILinqDbQueryable<T>() { _internal = _inter } };
        }
        /// <summary>
    ...
    {
        public static string GetTypeName<T>(string partition = null)
        {
    ...
    public class ILinqDbQueueQueryable<T> where T : new()
    {
        ...
        public void PutToQueue(T item)
        {
            ...
            var type_name = NameUtils.GetTypeName<T>();
            Command cm = CommandHelper.GetQueueCommand(_internal.Result, type_name, _internal._db.User, _internal._db.Pass);
            var bcm = CommandHelper.GetBytes(cm);
        ...
        public List<T> GetAllFromQueue()
        {
            ...
            var type_name = NameUtils.GetTypeName<T>();
            var cm = CommandHelper.GetQueueCommand(_internal.Result, type_name, _internal._db.User, _internal._db.Pass);
            var bcm = CommandHelper.GetBytes(cm);
            var bres = _internal._db.CallServer(bcm);
            var res_obj = ServerResultHelper.GetServerResult(bres);
            var res = new List<T>();
            ...
            while (current < res_obj.QueueData.Count())
            {
                ...
                var item = Ldb.DeserializeFromBytes<T>(SharedUtils.Decompress(item_bytes));
                res.Add(item);
            }
            return res;
        }
    ...
    public class ILinqDbNamedQueueQueryable<T> where T : new()
    {
        public IDbNamedQueueQueryable<T> _internal { get; set; }
        public string Name { get; set; }

        /// <summary>
        ///  Puts items to memory-queue and replace existing ones
        /// </summary>
        public void PutReplaceInNamedQueue(List<T> items)
        {
            var result = _internal._db.PutToNamedQueue(items);
        ...
        ///  Gets all items from memory-queue 
        /// </summary>
        public List<T> GetAllFromNamedQueue(bool delete_from_queue = false)
        {
            var result = _internal._db.GetFromNamedQueue<T>();
            result.Selector = Name;
            result.Int_null = delete_from_queue;
            _internal.Result.Add(result);
            ...
            res = Ldb.DeserializeFromBytes<List<T>>(SharedUtils.Decompress(res_obj.QueueData));
            return res;
        }
    }
    public class ILinqDbPartitionedQueryable<T> where T : new()
    {
        public ILinqDbQueryable<T> _queryable { get; set; }
        /// <summary>
        ///  Saves new item if Id = 0 and assigns new Id. Updates existing item if Id is existing's item's Id.
        /// </summary>
        public int Save(T item)
        {
            return _queryable.Save(item);
        }
        /// <summary>
        ...
        {
            _queryable.SaveNonAtomically(items, batchSize);
        }
        /// <summary>
        ...
        {
            _queryable.DeleteNonAtomically(ids, batchSize);
        }
        /// <summary>
        ///  Updates item's field with supplied value. Item is identified by Id.
        /// </summary>
        public void Update<TKey>(Expression<Func<T, TKey>> keySelector, int id, TKey value)
        {
            _queryable.Update(keySelector, id, value);
        }

        /// <summary>
        ///  Updates item's fields with supplied values. Item are identified by Id.
        /// </summary>
        public void Update<TKey>(Expression<Func<T, TKey>> keySelector, Dictionary<int, TKey> values)
        {
            _queryable.Update(keySelector, values);
        }

        /// <summary>
        ///  Updates any amount of items non-atomically, i.e. if it fails in the middle some items will be updated and some won't be.
        /// </summary>
        public void UpdateNonAtomically<TKey>(Expression<Func<T, TKey>> keySelector, Dictionary<int, TKey> values, int batchSize = 5000)
        {
            _queryable.UpdateNonAtomically(keySelector, values, batchSize);
        }
        ...
        {
            return _queryable.Select(predicate);
        }
        ...
        {
            return _queryable.Select(predicate, statistics);
        }
        ...
        {
            return _queryable.SelectNonAtomically(predicate, batchSize);
        }
        ...
        {
            return _queryable.SelectEntity();
        }
        /// <summary>
        ...
        {
            return _queryable.SelectEntity(statistics);
        }
        ...
        {
            return _queryable.SelectEntityNonAtomically(batchSize);
        }
        ...
        {
            return _queryable.AtomicIncrement(keySelector, value, new_item_if_doesnt_exist, only_if_old_equal);
        }
        /// <summary>
        ...
        {
            _queryable.AtomicIncrement2Props(keySelector1, keySelector2, value1, value2, new_item_if_doesnt_exist);
        }
        /// <summary>
        ///  Applies between condition to the result set.
        /// </summary>
        public ILinqDbQueryable<T> Between<TKey>(Expression<Func<T, TKey>> keySelector, TKey from, TKey to, BetweenBoundaries boundaries = BetweenBoundaries.BothInclusive)
        {
            return _queryable.Between(keySelector, from, to, boundaries);
        }

        /// <summary>
        ///  Applies intersect condition to the result set.
        /// </summary>
        public ILinqDbQueryable<T> Intersect<TKey>(Expression<Func<T, TKey>> keySelector, HashSet<TKey> set)
        {
            return _queryable.Intersect(keySelector, set);
        }
        ...
        ///  Applies intersect condition to the result set.
        /// </summary>
        public ILinqDbQueryable<T> Intersect<TKey>(Expression<Func<T, TKey>> keySelector, List<TKey> set)
        {
            return _queryable.Intersect(keySelector, set);
        }
        ...
        {
            return _queryable.Search(keySelector, search_query, start_step, steps);
        }
        /// <summary>
        ...
        {
            return _queryable.SearchPartial(keySelector, search_query);
        }
        /// <summary>
        ...
        {
            return _queryable.SearchTimeLimited(keySelector, search_query, maxSearchTimeInMs);
        }
        /// <summary>
        /// Vector space search for k-nearest neighbours. Uses aproximate on-disk algorithm.
        /// </summary>
        public ILinqDbQueryable<T> SearchNeighbours<TKey>(Expression<Func<T, TKey>> keySelector, float[] vector, int k)
        {
            return _queryable.SearchNeighbours(keySelector, vector, k);
        }
        /// <summary>
        ...
        {
            return _queryable.OrderBy(keySelector);
        }
        /// <summary>
        ...
        {
            return _queryable.OrderByDescending(keySelector);
        }
    }
    public class ILinqDbQueryable<T> where T : new()
    {
        ...
        public int Save(T item)
        {
            if (item != null && !_internal._db.IsDist)
            {
                var sid_prop = item.GetType().GetProperty("Sid");
                if (sid_prop != null)
                {
                    throw new LinqDbException("Linqdb: type cannot have Sid property, it is reserved for distributed table.");
                }
            }
            var type_name = NameUtils.GetTypeName<T>(_internal.Partition);
            if (_internal.LDBTransaction == null)
            {
                var def = _internal._db.GetTableDefinition<T>(_internal.Partition);
                var result = _internal._db.Save(item, def.Item1, def.Item2);
                _internal.Result.Add(result);
                Command cm = CommandHelper.GetCommand(_internal.Result, def.Item1, def.Item2, type_name, _internal._db.User, _internal._db.Pass);
                var bcm = CommandHelper.GetBytes(cm);
                var bres = _internal._db.CallServer(bcm);
                var res_obj = ServerResultHelper.GetServerResult(bres);
            ...
            else
            {
                ...
                if (!_internal.LDBTransaction.defs.ContainsKey(type_name))
                {
                    _internal.LDBTransaction.defs[type_name] = def;
                }
                if (_internal.LDBTransaction.data_to_save.ContainsKey(type_name))
                {
                    _internal.LDBTransaction.data_to_save[type_name].Add(item);
                }
                else
                {
                    _internal.LDBTransaction.data_to_save[type_name] = new List<object>() { item };
                }
                return (int)old_id;
            }
        }
        ...
        public void SaveNonAtomically(List<T> items, int batchSize = 5000)
        {
            ...
            var first = items[0];
            var sid_prop = first.GetType().GetProperty("Sid");
            if (sid_prop != null && !_internal._db.IsDist)
            {
                throw new LinqDbException("Linqdb: type cannot have Sid property, it is reserved for distributed table.");
            }
        ...
        public void SaveBatch(List<T> items)
        {
            ...
            else
            {
                ...
                if (!_internal.LDBTransaction.data_to_save.ContainsKey(type_name))
                {
                    _internal.LDBTransaction.data_to_save[type_name] = new List<object>();
                }
                _internal.LDBTransaction.data_to_save[type_name].AddRange(items.Cast<object>());
            }
        }
        /// <summary>
        ///  Record count.
        ...
        public int Count()
        {
            ...
            var def = _internal._db.GetTableDefinition<T>(_internal.Partition);
            var type_name = NameUtils.GetTypeName<T>(_internal.Partition);
            var cm = CommandHelper.GetCommand(_internal.Result, def.Item1, def.Item2, type_name, _internal._db.User, _internal._db.Pass);
            var bcm = CommandHelper.GetBytes(cm);
            var bres = _internal._db.CallServer(bcm);
            var res_obj = ServerResultHelper.GetServerResult(bres);
        ...
        public void Delete(int id)
        {
            ...
            else
            {
                ...
                if (_internal.LDBTransaction.data_to_delete.ContainsKey(type_name))
                {
                    _internal.LDBTransaction.data_to_delete[type_name].UnionWith(result.DeleteIds);
                }
                else
                {
                    _internal.LDBTransaction.data_to_delete[type_name] = result.DeleteIds;
                }
            }
        }
        /// <summary>
        ...
        public void Update<TKey>(Expression<Func<T, TKey>> keySelector, int id, TKey value)
        {
            ...
            if (_internal.LDBTransaction == null)
            {
                var result = _internal._db.Update(keySelector, new Dictionary<int, TKey>() { { id, value } });
                _internal.Result.Add(result);
            ...
            else
            {
                ...
                if (_internal.LDBTransaction.data_to_update.ContainsKey(type_name))
                {
                    _internal.LDBTransaction.data_to_update[type_name].Add(new KeyValuePair<string, Dictionary<int, byte[]>>(result.Selector, result.UpdateData));
                }
                else
                {
                    _internal.LDBTransaction.data_to_update[type_name] = new List<KeyValuePair<string, Dictionary<int, byte[]>>>();
                    _internal.LDBTransaction.data_to_update[type_name].Add(new KeyValuePair<string, Dictionary<int, byte[]>>(result.Selector, result.UpdateData));
                }
            }
        }
        ...
        public List<R> Select<R>(Expression<Func<T, R>> predicate, LinqdbSelectStatistics statistics)
        {
            ...
            if (res_obj.QueueData != null)
            {
                statistics.Distances = ServerSharedData.SharedUtils.SimpleIntDoubleCodec.FromBytes(res_obj.QueueData);
            }
        ...
        public List<R> SelectNonAtomically<R>(Expression<Func<T, R>> predicate, int batchSize = 3000)
        {
            ...
            for (int i = 0; ; i++)
            {
                var currentIds = ids.Skip(i * batchSize).Take(batchSize).Select(f => f).ToList();
                if (!currentIds.Any())
                {
                    return results;
                }
                ...
                cm = CommandHelper.GetCommand(_internal.Result, def.Item1, def.Item2, type_name, _internal._db.User, _internal._db.Pass);
        ...
        public ILinqDbQueryable<T> Intersect<TKey>(Expression<Func<T, TKey>> keySelector, List<TKey> set)
        {
            var result = _internal._db.Intersect(keySelector, new HashSet<TKey>(set.Select(f => f)));
            _internal.Result.Add(result);
            return this;
        }
        ...
        /// </summary>
        public ILinqDbQueryable<T> SearchBitFunnel<TKey>(Expression<Func<T, TKey>> keySelector, string search_query, int? stop_res_count = null)
        {
            var result = _internal._db.SearchBitfunnel(keySelector, search_query, stop_res_count);
            _internal.Result.Add(result);
            return this;
        }
        /// <summary>
        ...
        public ILinqDbQueryable<T> SearchNeighbours<TKey>(Expression<Func<T, TKey>> keySelector, float[] vector, int k)
        {
            var result = _internal._db.SearchNeighbours(keySelector, vector, k);
            _internal.Result.Add(result);
            return this;
        }
        /// <summary>
        ...
        public void CreatePropertyMemoryIndex<TKey>(Expression<Func<T, TKey>> valuePropertySelector)
        {
            ...
            {
                throw new LinqDbException("Linqdb: memory index can't be created on string properties.");
            }

            var result = _internal._db.CreatePropertyIndex(valuePropertySelector, false, 0, 0);
            _internal.Result.Add(result);
            var type_name = NameUtils.GetTypeName<T>(_internal.Partition);
        ...
        {
            if (bitfunnel_size % 8 != 0 || bitfunnel_size <= 0)
            {
                throw new LinqDbException("Linqdb: bitfunnel_size must be a multiple of 8.");
            }

            if (batch_size % 1024 != 0 || batch_size <= 0)
            {
                throw new LinqDbException("Linqdb: batch_size must be a multiple of 1024.");
            }

            if (typeof(TKey) != typeof(string))
            {
                throw new LinqDbException("Linqdb: bitfunnel index can be created and removed on string properties only.");
            }

            var result = _internal._db.CreatePropertyIndex(valuePropertySelector, true, bitfunnel_size, batch_size);
            _internal.Result.Add(result);
            var type_name = NameUtils.GetTypeName<T>(_internal.Partition);
            var def = _internal._db.GetTableDefinition<T>();
            Command cm = CommandHelper.GetCommand(_internal.Result, def.Item1, def.Item2, type_name, _internal._db.User, _internal._db.Pass);
            ...
            var bres = _internal._db.CallServer(bcm);
            var res_obj = ServerResultHelper.GetServerResult(bres);
            return res_obj.Count / (double)100;
        }
        /// <summary>
        ...
        {
            if (typeof(TKey1) == typeof(string) || typeof(TKey2) == typeof(string))
            {
                throw new LinqDbException("Linqdb: memory index can't be created on string properties.");
            }
        ...
        public void RemovePropertyMemoryIndex<TKey>(Expression<Func<T, TKey>> valuePropertySelector)
        {
            if (typeof(TKey) == typeof(string))
            {
                throw new LinqDbException("Linqdb: memory index can't be removed on string properties.");
            }

            var result = _internal._db.RemovePropertyIndex(valuePropertySelector, false);
            _internal.Result.Add(result);
        ...
        public void RemoveBitFunnelMemoryIndex<TKey>(Expression<Func<T, TKey>> valuePropertySelector)
        {
            ...
            var result = _internal._db.RemovePropertyIndex(valuePropertySelector, true);
            _internal.Result.Add(result);
        ...
        public ILinqDbOrderedQueryable<T> OrderBy<TKey>(Expression<Func<T, TKey>> keySelector)
        {
            ...
            _internal.Result.Add(result);
            var _inter = new IDbOrderedQueryable<T>() { Result = _internal.Result, _db = _internal._db, Partition = _internal.Partition };
